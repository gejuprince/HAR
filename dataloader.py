...
class HARDataSet(Dataset):
    def __init__(self, root_dir, transform=None, sequence_length=10):
        ...
        self.samples = []
        self.action_to_idx = {'down': 0, 'grab': 1, 'walk': 2}  # Define your mappings here
            ...
            for frame_folder in os.listdir(action_path):
                ...
                if len(frames) >= sequence_length:
                    ...
                    for i in range(0, len(frames) - sequence_length + 1, 1):
                        self.samples.append((frames[i:i + sequence_length], imu_path, action))

    def __len__(self):
        return len(self.samples)

    def __getitem__(self, idx):
        frames_path, imu_path, action = self.samples[idx]
        frames = [Image.open(frame) for frame in frames_path]
        if self.transform:
            frames = [self.transform(frame) for frame in frames]

        imu_data = pd.read_csv(imu_path).drop(columns=['time_stamp'])
        imu_data = imu_data.iloc[:len(frames_path)]

        action_idx = self.action_to_idx[action]  # Convert action name to integer
        action_tensor = torch.tensor(action_idx, dtype=torch.long)  # Convert to tensor

        # Rearrange tensor dimensions to [C, num_frames, H, W]
